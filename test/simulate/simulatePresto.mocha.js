...
describe("simulate Presto", () => {
  it("works in advanced case", () => {
    ...
    expect(queryPlan[0]).to.equal(sane`
      ...
      SUM(IF(("cut" IS NOT DISTINCT FROM 'good'),"price",0)) AS "PriceGoodCut"
      FROM "diamonds"
      WHERE ("color" IS NOT DISTINCT FROM 'D')
      GROUP BY ''
    `);
    ...
    expect(queryPlan[2]).to.equal(sane`
      SELECT
      (DATE_TRUNC('day',("time" AT TIME ZONE 'UTC' AT TIME ZONE 'America/Los_Angeles')) AT TIME ZONE 'America/Los_Angeles' AT TIME ZONE 'UTC') AS "Timestamp",
      SUM("price") AS "TotalPrice"
      FROM "diamonds"
      WHERE (("color" IS NOT DISTINCT FROM 'D') AND ("cut" IS NOT DISTINCT FROM 'some_cut'))
      GROUP BY 1
    ...
    expect(queryPlan[3]).to.equal(sane`
      ...
      FROM "diamonds"
      WHERE ((("color" IS NOT DISTINCT FROM 'D') AND ("cut" IS NOT DISTINCT FROM 'some_cut')) AND (TIMESTAMP '2015-03-13 07:00:00'<="time" AND "time"<TIMESTAMP '2015-03-14 07:00:00'))
      GROUP BY 1
      ORDER BY "Count" DESC
      LIMIT 3
    `);
  ...
  it("works with BOOLEAN bucket", () => {
    ...
    expect(queryPlan[0]).to.equal(sane`
      SELECT
      ("color" IS NOT DISTINCT FROM 'A') AS "ColorIsA",
      COUNT(*) AS "Count"
      FROM "diamonds"
      GROUP BY 1
      ORDER BY "Count" DESC
  ...
  it("works multi-dimensional GROUP BYs", () => {
    ...
    expect(queryPlan[1]).to.equal(sane`
      ...
      COUNT(*) AS "Count"
      FROM "diamonds"
      WHERE (("color" IS NOT DISTINCT FROM 'some_color') AND ("cut" IS NOT DISTINCT FROM 'some_cut'))
      GROUP BY 1
      ORDER BY "Count" DESC
      LIMIT 3
    `);
